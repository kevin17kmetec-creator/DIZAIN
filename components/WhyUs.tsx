          ...
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-px bg-[var(--border-color)] border border-[var(--border-color)]">
            {t.whyUs.items.map((item, index) => (
              <motion.div 
                ...
                className={`bg-[var(--bg-secondary)] p-12 hover:bg-[var(--bg-tertiary)] transition-colors duration-500 flex flex-col justify-between aspect-square ${index === 2 ? 'md:col-span-2 md:aspect-[2/1]' : ''}`}
              >
                ...
                <div className="text-right mt-8">
                    <span className="text-6xl font-display text-[var(--text-main)]/5 font-bold">0{index + 1}</span>
                </div>
              </motion.div>
            ))}
          </div>
